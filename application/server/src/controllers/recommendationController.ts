 ...
 */
export const fetchGamesData = async (req: Request, res: Response): Promise<Response> => {
  try {
    const games = await fetchGames();
    return res.status(200).json(games);
  } catch (error) {
    console.error('[fetchGamesData] Error fetching games data:', (error as Error).message);
    return res.status(500).json({
      message: 'Error fetching games data. Please try again later.',
...
export const fetchUserData = async (req: Request, res: Response): Promise<Response> => {
  ...
  try {
    ...
    if (!userData) {
      return res.status(404).json({ message: `User data not found for ID: ${userId}` });
    }

    return res.status(200).json(userData);
  } catch (error) {
    console.error('[fetchUserData] Error fetching user data:', (error as Error).message);
    return res.status(500).json({
...
export const generateGameEmbeddingsWithPCA = async (
  ...
  try {
    ...
    console.error(
      '[generateGameEmbeddingsWithPCA] Error generating game embeddings:',
      (error as Error).message
    );
    return res.status(500).json({
      message: 'Error generating game embeddings. Please try again later.',
      error: (error as Error).message,
    });
  }
};
